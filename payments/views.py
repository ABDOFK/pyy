...
def payment_dashboard(request):
    ...
    if user.factures_recues.exists():
        ...
        montant_total = (
            factures_recues.aggregate(Sum("montant_ttc"))["montant_ttc__sum"] or 0
        )
        montant_paye = (
            factures_recues.filter(statut=FactureStatus.PAID).aggregate(
                Sum("montant_ttc")
            )["montant_ttc__sum"]
            or 0
        )
        ...
        stats_client = {
            ...
            "factures_payees": factures_recues.filter(
                statut=FactureStatus.PAID
            ).count(),
            "factures_en_attente": factures_recues.filter(
                statut=FactureStatus.SENT
            ).count(),
            "factures_en_retard": factures_recues.filter(
                statut=FactureStatus.OVERDUE
            ).count(),
            "montant_total": montant_total,
            "montant_paye": montant_paye,
            "reste_a_payer": montant_total - montant_paye,  # ← NOUVEAU CALCUL
        }
    else:
        stats_client = None
    ...
    if user.factures_emises.exists():
        ...
        stats_freelance = {
            ...
            "factures_payees": factures_emises.filter(
                statut=FactureStatus.PAID
            ).count(),
            "factures_en_attente": factures_emises.filter(
                statut=FactureStatus.SENT
            ).count(),
            "montant_total": factures_emises.aggregate(Sum("montant_freelance"))[
                "montant_freelance__sum"
            ]
            or 0,
            "montant_recu": factures_emises.filter(statut=FactureStatus.PAID).aggregate(
                Sum("montant_freelance")
            )["montant_freelance__sum"]
            or 0,
        }
    else:
    ...
    # Paiements récents
    recent_payments = Paiement.objects.filter(facture__client=user).order_by(
        "-date_paiement"
    )[:5]
...
def create_invoice_from_mission(request, mission_id):
    ...
    if not (
        ...
        messages.error(
            request, "Vous n'êtes pas autorisé à créer une facture pour cette mission."
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Facture {facture.reference_facture} créée avec succès!"
            )
...
def invoice_detail(request, pk):
    ...
    context = {
        ...
        "can_release_escrow": escrow_account
        and not escrow_account.est_libere
        and facture.client == request.user,
    }
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if method == "ESCROW":
                ...
                messages.error(
                    request,
                    f"Méthode de paiement {method} non disponible pour le moment.",
                )
    else:
...
def bank_transfer_payment(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "Votre déclaration de virement a été enregistrée."
            )
            return redirect("payments:invoice_detail", pk=pk)
    else:
...
def escrow_payment(request, pk):
    ...
    if request.method == "POST":
        ...
        escrow_account = EscrowAccount.objects.create(
            facture=facture, montant_bloque=facture.montant_ttc
        )
...
def release_escrow(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Les fonds ont été libérés vers {facture.freelance.username}."
            )
            return redirect("payments:invoice_detail", pk=pk)
    else:
