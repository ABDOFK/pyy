...
# users/views.py - Correction de la fonction login_view
...
def login_view(request):
    ...
    # Initialiser next_url dès le début
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                # Utiliser next_url qui est maintenant défini
    ...
    # next_url est maintenant toujours défini
...
def update_freelance_profile(request):
    ...
    if request.method == "POST":
        form = FreelanceProfileForm(
            request.POST, request.FILES or None, instance=profile_instance
        )
        if form.is_valid():
    ...
    context = {"form": form, "profile_type": "Freelance"}
...
def update_client_profile(request):
    ...
    context = {"form": form, "profile_type": "Client"}
    return render(request, "users/profile_form.html", context)
...
def profile_view(request):
    ...
    evaluations_recues = Evaluation.objects.filter(evalue=request.user).order_by(
        "-date_evaluation"
    )
...
def freelance_search_view(request):
    ...
    queryset = (
        FreelanceProfile.objects.filter(titre_professionnel__isnull=False)
        .exclude(titre_professionnel__exact="")
        .select_related("user")
        .order_by("-id")
    )
...
def freelance_profile_view(request, user_id):
    ...
    evaluations_recues = (
        Evaluation.objects.filter(evalue=freelance_user)
        .select_related("evaluateur", "mission")
        .order_by("-date_evaluation")[:5]
    )

    if evaluations_recues:
        note_moyenne = sum(eval.note for eval in evaluations_recues) / len(
            evaluations_recues
        )
    ...
    missions_terminees = Mission.objects.filter(
        freelance_assigne=freelance_user, statut__in=["COMPLETED", "CLOSED"]
